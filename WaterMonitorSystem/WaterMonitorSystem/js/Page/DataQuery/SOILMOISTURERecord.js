...
function InitTimeControl() {
    ...
    $("#txt_StartTime").val(e.DateAdd('y', -1).Format("yyyy-MM-dd 00:00"));
        ...
        success: function (responseText) {
            ...
            if (data.Result == true) {
                ...
                for (var i = 0; i < data.Records.length; i++) {
                    if (xAxis_categories.indexOf(data.Records[i].AcqTime) == -1)
                        xAxis_categories.push(data.Records[i].AcqTime);
                ...
                for (var i = 0; i < series_name.length; i++) {
                    ...
                    for (var j = 0; j < xAxis_categories.length; j++) {
                        ...
                        for (var k = 0; k < data.Records.length; k++) {
                            if (data.Records[k].AcqTime == xAxis_categories[j] && data.Records[k].StationID == series_name[i]) {
                                myMap1['data'].push(parseFloat(data.Records[k].SoilMoisture1));
                                myMap2['data'].push(parseFloat(data.Records[k].SoilMoisture2));
                                myMap3['data'].push(parseFloat(data.Records[k].SoilMoisture3));
                                flag = true;
                                break;
                            }
                        }
                ...
                $('#divChart1').highcharts({
                    ...
                    yAxis: { title: { text: '土壤水分1（%）' }, labels: { formatter: function () { return this.value + '%' } } },
                ...
                $('#divChart2').highcharts({
                    ...
                    xAxis: { categories: xAxis_categories },
                    yAxis: { title: { text: '土壤水分2（%）' }, labels: { formatter: function () { return this.value + '%' } } },
                    tooltip: { crosshairs: true, shared: true },
                    plotOptions: { spline: { marker: { radius: 4, lineColor: '#666666', lineWidth: 1 } } },
                    series: series2
                });
                ...
                $('#divChart3').highcharts({
                    ...
                    xAxis: { categories: xAxis_categories },
                    yAxis: { title: { text: '土壤水分3（%）' }, labels: { formatter: function () { return this.value + '%' } } },
                    tooltip: { crosshairs: true, shared: true },
                    plotOptions: { spline: { marker: { radius: 4, lineColor: '#666666', lineWidth: 1 } } },
                    series: series3
                });
...
function QueryCurrentPageData(pageNumber, pageSize) {
    var startIndex = (pageNumber - 1) * pageSize + 1;
    $.ajax({
        url: "../WebServices/DataQueryNewService.asmx/GetSOILMOISTURERecords",
        data: { "loginIdentifer": window.parent.guid, "operateIdentifer": operateIdentifer, "startIndex": startIndex, "count": pageSize },
        type: "GET",
        dataType: "text",
        cache: false,
...
function Btn_Excel_Click() {
    ...
    $.ajax(
    {
        url: "../WebServices/DataQueryNewService.asmx/ExportSOILMOISTURERecords",
        type: "GET",
